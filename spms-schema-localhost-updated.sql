...
CREATE TABLE STUDENT_ACHIEVED_CO_PLO (
    ...
    semesterSeason VARCHAR(50) NOT NULL,
    semesterYear YEAR NOT NULL,
    COAchieved CHAR(255), # change COAchieved to Boolean(maybe)
    totalMarksReceivedForCO DOUBLE,
    sectionNumber TINYINT UNSIGNED,
    PLOAchieved CHAR(255), ## change PLOachieved to Boolean(maybe)
...
## find AVG PLO mapped to the courses above
SELECT AVG(totalPLOID) as avgPLOID
FROM (SELECT courseID, COUNT(DISTINCT PLOID) as totalPLOID
	  FROM CO_PLO_MAPPING
	  WHERE courseID IN
...
## total students evaluated by OBE model
SELECT COUNT(S.sAccountID)
FROM STUDENT AS S, STUDENT_ENROLLMENT AS SE
WHERE S.sAccountID = SE.sAccountID 
		AND SE.enrolledCourseID IN 
        (SELECT DISTINCT C.courseID AS obeCourses
		FROM COURSE AS C, PROGRAM_LEARNING_OUTCOME AS PLO, CO_PLO_MAPPING AS CO_PLO_MAP 
		WHERE C.courseID = CO_PLO_MAP.courseID AND PLO.PLOID = CO_PLO_MAP.PLOID);
